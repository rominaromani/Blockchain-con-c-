...
public:
    ...
    //Operator =
    CircularArray<T>& operator=(const CircularArray<T> &other){
        if(this != &other){
            delete[] array;
    ...
    bool remove(T dato){
        if (is_empty()){ 
            ...
            int pos = find(dato);
            if(pos!=-1){
                while(pos != next(back)){ //empuja elementos necesarios a la izquierda
                    array[pos] = array[next(pos)];
                    pos = next(pos);
                }
                back = prev(back); // se retrocede el back
                size--; // se disminuye el size
                return true;
            }
            else{
