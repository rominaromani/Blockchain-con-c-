...
public:
    ...
    bool addBlock(T bloque){
        ...
        cout << "Block " << countBlock << ": " << (test? "añadido": "no añadido (minar bloques)") << " -> nonce: " << bloque->nonce << endl;
        countBlock++;
    ...
    void addToOperations(T block){
        ...
        for (int i=0; i<block->transferencias.get_size(); i++){ // O(4) para el caso de 4 transferencias
        // Hash
            // fechas
            if (hashFecha->get(block->transferencias[i]->fecha.getString()) == nullptr){ // O(1) get del hash
                CircularArray<Transferencia*> *arr = new CircularArray<Transferencia*>();
                arr->push_back(block->transferencias[i]); // O(1) push_back() en un array circular, en caso la cantidad de transferencias sea mayor a 10, se hace resize O(n)
                hashFecha->set(block->transferencias[i]->fecha.getString(), arr); // O(1) set en un hash
            }
            else {
                hashFecha->get(block->transferencias[i]->fecha.getString())->push_back(block->transferencias[i]); // O(1) obteniendo el value del hash, un array, y O(1) push_back() en el array circular
            }
            // emisor
            if (hashEmisor->get(block->transferencias[i]->emisor) == nullptr){ // O(1) get del hash
                CircularArray<Transferencia*> *arr = new CircularArray<Transferencia*>();
                arr->push_back(block->transferencias[i]); // O(1) push_back() en un array circular, en caso la cantidad de transferencias sea mayor a 10, se hace resize O(n)
                hashEmisor->set(block->transferencias[i]->emisor, arr); // O(1) set en un hash
            }
            else {
                hashEmisor->get(block->transferencias[i]->emisor)->push_back(block->transferencias[i]); // O(1) obteniendo el value del hash, un array, y O(1) push_back() en el array circular
            }
            // receptor
            if (hashReceptor->get(block->transferencias[i]->receptor) == nullptr){ // O(1) get del hash
                CircularArray<Transferencia*> *arr = new CircularArray<Transferencia*>();
                arr->push_back(block->transferencias[i]); // O(1) push_back() en un array circular, en caso la cantidad de transferencias sea mayor a 10, se hace resize O(n)
                hashReceptor->set(block->transferencias[i]->receptor, arr); // O(1) set en un hash
            }
            else {
                hashReceptor->get(block->transferencias[i]->receptor)->push_back(block->transferencias[i]); // O(1) obteniendo el value del hash, un array, y O(1) push_back() en el array circular
            }
            // monto
            if (hashMonto->get(block->transferencias[i]->monto) == nullptr){ // O(1) get del hash
                CircularArray<Transferencia*> *arr = new CircularArray<Transferencia*>();
                arr->push_back(block->transferencias[i]); // O(1) push_back() en un array circular, en caso la cantidad de transferencias sea mayor a 10, se hace resize O(n)
                hashMonto->set(block->transferencias[i]->monto, arr); // O(1) set en un hash
            }
            else {
                hashMonto->get(block->transferencias[i]->monto)->push_back(block->transferencias[i]); // O(1) obteniendo el value del hash, un array, y O(1) push_back() en el array circular
            }

        // Tries
            trieEmisor->insert(block->transferencias[i]->emisor, block->transferencias[i]); // O(m), teniendo en cuenta que es constante al tamaño de la palabra, emisor
            trieReceptor->insert(block->transferencias[i]->receptor, block->transferencias[i]); // O(m), teniendo en cuenta que es constante al tamaño de la palabra, receptor

        // Heap
            max.push(block->transferencias[i]->monto); // O(log n) por el heapify_up, si hace resize O(n)
            min.push(block->transferencias[i]->monto); // O(log n) por el heapify_up, si hace resize O(n)
            maxFecha.push(block->transferencias[i]->fecha); // O(log n) por el heapify_up, si hace resize O(n)
            minFecha.push(block->transferencias[i]->fecha); // O(log n) por el heapify_up, si hace resize O(n)

        //AVL
            avlMonto->insert(block->transferencias[i]->monto, block->transferencias[i]); // O(log n) por el insert 
            avlFecha->insert(block->transferencias[i]->fecha, block->transferencias[i]); // O(log n) por el insert
        }
    }

    void removeFromOperations(T block){ // elimina todos los datos del bloque de las estructuras de búsqueda
        for (int i=0; i<block->transferencias.get_size(); i++){ // O(4) para el caso de 4 transferencias
        // Hash
            // fechas
            if (hashFecha->get(block->transferencias[i]->fecha.getString()) != nullptr && (hashFecha->get(block->transferencias[i]->fecha.getString()))->get_size() == 1){ // O(1) para verificar que existe el dato, O(1) para verificar si solo tiene 1 transferencia
                hashFecha->remove(block->transferencias[i]->fecha.getString()); // O(1) para eliminar el key y value del hash
            }
            else if (hashFecha->get(block->transferencias[i]->fecha.getString()) != nullptr) { // O(1) para verificar que existe el dato
                hashFecha->get(block->transferencias[i]->fecha.getString())->remove(block->transferencias[i]); // O(k): se optiene el array que guarda las transferencias y se elimina únicamente la transferencia solicitada
            }
            // emisor
            if (hashEmisor->get(block->transferencias[i]->emisor) != nullptr && hashEmisor->get(block->transferencias[i]->emisor)->get_size() == 1){ // O(1) para verificar que existe el dato, O(1) para verificar si solo tiene 1 transferencia
                hashEmisor->remove(block->transferencias[i]->emisor); // O(1) para eliminar el key y value del hash
            }
            else if (hashEmisor->get(block->transferencias[i]->emisor) != nullptr){ // O(1) para verificar que existe el dato
                (hashEmisor->get(block->transferencias[i]->emisor))->remove(block->transferencias[i]); // O(k): se optiene el array que guarda las transferencias y se elimina únicamente la transferencia solicitada
            }
            // receptor
            if (hashReceptor->get(block->transferencias[i]->receptor) != nullptr && hashReceptor->get(block->transferencias[i]->receptor)->get_size() == 1){ // O(1) para verificar que existe el dato, O(1) para verificar si solo tiene 1 transferencia
                hashReceptor->remove(block->transferencias[i]->receptor); // O(1) para eliminar el key y value del hash
            }
            else if (hashReceptor->get(block->transferencias[i]->receptor) != nullptr) { // O(1) para verificar que existe el dato
                hashReceptor->get(block->transferencias[i]->receptor)->remove(block->transferencias[i]); // O(k): se optiene el array que guarda las transferencias y se elimina únicamente la transferencia solicitada
            }
            // monto
            if (hashMonto->get(block->transferencias[i]->monto) != nullptr && hashMonto->get(block->transferencias[i]->monto)->get_size() == 1){ // O(1) para verificar que existe el dato, O(1) para verificar si solo tiene 1 transferencia
                hashMonto->remove(block->transferencias[i]->monto); // O(1) para eliminar el key y value del hash
            }
            else if (hashMonto->get(block->transferencias[i]->monto) != nullptr) { // O(1) para verificar que existe el dato
                hashMonto->get(block->transferencias[i]->monto)->remove(block->transferencias[i]); // O(k): se optiene el array que guarda las transferencias y se elimina únicamente la transferencia solicitada
            }

        // Tries
            trieEmisor->remove(block->transferencias[i]->emisor); // O(m), m es el tamaño de la palabra, emisor
            trieReceptor->remove(block->transferencias[i]->receptor); // O(m), m es el tamaño de la palabra, emisor

        // Heap
            max.remove(block->transferencias[i]->monto); // O(log n) remove del heap
            min.remove(block->transferencias[i]->monto); // O(log n) remove del heap
            maxFecha.remove(block->transferencias[i]->fecha); // O(log n) remove del heap
            minFecha.remove(block->transferencias[i]->fecha); // O(log n) remove del heap

        //AVL
            avlMonto->remove(block->transferencias[i]->monto); // O(log n) remove del avl, por el auto balanceo
            avlFecha->remove(block->transferencias[i]->fecha); // O(log n) remove del avl, por el auto balanceo
        }
    }
